...
class ChainAgent:
    ...
    async def build(self, req: ChainBuildRequest, known_nodes: list[str] | None = None):
        ...
        for depth in range(1, req.max_depth + 1):
            ...
            # ── 跨链桥梁引导：多主体时强制要求 LLM 找宏观中介 ──
            cross_chain_note = ""
            if is_batch and depth == 1 and len(subjects) > 1:
                cross_chain_note = (
                    f"\n## ⚠️ 跨链桥梁要求（极其重要！必须执行！）\n"
                    f"你正在同时为 **{'、'.join(subjects)}** 构建产业链。\n"
                    f"这些主体看似属于不同领域，但它们之间**一定存在共同的宏观桥梁因子**。\n\n"
                    f"**你必须输出至少 1-2 个「桥梁节点」**，这些节点同时与多个主体相关联，例如：\n"
                    f"- **宏观因子**（node_type=\"macro\"）：美元指数、利率、通胀预期、地缘风险、全球流动性等\n"
                    f"- **共同定价因子**：CPI、PPI、大宗商品指数、VIX恐慌指数、实际利率等\n"
                    f"- **共同需求端**：工业需求、消费需求、投资需求、央行储备需求等\n"
                    f"- **共同交易市场**：期货市场、COMEX、LME、上期所等\n"
                    f"- **跨界企业**：同时涉及多个主体的大型企业\n\n"
                    f"**桥梁节点的 links 必须同时连接到不同主体的产业链中**。\n"
                    f"例如对于「原油、黄金」：\n"
                    f"- 「美元指数」应同时连到原油（原油以美元计价→美元升值则油价承压）和黄金（黄金与美元负相关）\n"
                    f"- 「通胀预期」应同时连到原油（油价是通胀前瞻指标）和黄金（黄金是抗通胀资产）\n"
                    f"- 「地缘风险」应同时连到原油（供给担忧推升油价）和黄金（避险需求推升金价）\n\n"
                    f"**如果你不输出跨链桥梁节点，那多主体同时构建就失去了意义，等于各画各的！**\n"
                )
            ...
            prompt = CHAIN_BUILD_PROMPT.format(
                ...
                cross_chain_note=cross_chain_note,
    ...
    async def expand_all(self, targets: list[tuple[str, str]], existing_nodes: list[str], existing_links: list[dict] | None = None, max_depth: int = 1):
        ...
        5. 展开完成后自动检测连通分量，发现断连则用 CHAIN_BRIDGE_PROMPT 找桥梁

        Args:
            targets: [(node_name, direction), ...] direction = "upstream" | "downstream" | "both"
            existing_nodes: 已有节点名列表
            existing_links: 已有的边列表 [{"source": "A", "target": "B", "relation": "..."}]
        ...
        # ── 第四阶段：自动桥梁检测 ── 展开完成后检测连通分量，发现断连则自动找桥梁
        all_node_names = list(seen_nodes)
        all_link_tuples = [(k.split("->")[0], k.split("->")[1]) for k in seen_links if "->" in k]
        # 加上前端传来的已有边（place-node 阶段建立的关系）
        if existing_links:
            for el in existing_links:
                s, t = el.get("source", ""), el.get("target", "")
                if s and t:
                    all_link_tuples.append((s, t))

        if len(all_node_names) >= 4:  # 至少 4 个节点才值得检测
            components = _find_connected_components(all_node_names, all_link_tuples)
            if len(components) > 1:
                logger.info(
                    f"expand_all: 展开后检测到 {len(components)} 个断连子图 "
                    f"(大小: {[len(c) for c in components]}), 启动自动桥梁发现"
                )
                yield {
                    "event": "bridge_start",
                ...
                comp_desc_parts = []
                for i, comp in enumerate(components):
                    comp_nodes_sorted = sorted(comp)[:10]
                    suffix = f"... 等共 {len(comp)} 个节点" if len(comp) > 10 else ""
                    comp_desc_parts.append(
                        f"### 板块 {i+1}（{len(comp)} 个节点）\n{', '.join(comp_nodes_sorted)}{suffix}"
                    )

                components_description = "\n\n".join(comp_desc_parts)

                # 已有边精简描述：优先用前端传来的真实 relation，再补充本次新边
                bridge_existing_lines = []
                if existing_links:
                    for el in existing_links[:30]:
                        s = el.get("source", "")
                        t = el.get("target", "")
                        r = el.get("relation", "?")
                        if s and t:
                            bridge_existing_lines.append(f"- {s} →[{r}]→ {t}")
                for k in list(seen_links)[:30]:
                    parts = k.split("->")
                    if len(parts) == 2:
                        bridge_existing_lines.append(f"- {parts[0]} → {parts[1]}")
                # 去重
                bridge_existing_lines = list(dict.fromkeys(bridge_existing_lines))
                bridge_existing = "\n".join(bridge_existing_lines[:60])[:2000]

                min_bridges = len(components) - 1
                ...
                try:
                    ...
                    bridge_links = []
                    existing_link_keys = set(seen_links)
                    for lk in parsed.get("links", []):
                        source = _normalize_name(lk.get("source", ""))
                        target = _normalize_name(lk.get("target", ""))
                        if not source or not target or source == target:
                        ...
                        if key in existing_link_keys or rev_key in existing_link_keys:
                            continue
                        ...
                        bridge_new_nodes = []
                        for nm in [source, target]:
                            if nm not in seen_nodes:
                                seen_nodes.add(nm)
                                bridge_node = ChainNode(
                                    id=f"n_{nm}", name=nm,
                                    node_type="macro",  # 桥梁节点默认为宏观因子
                                    impact="neutral", impact_score=0.0, depth=0,
                                    summary=f"跨链桥梁节点：{nm}",
                                )
                                bridge_new_nodes.append(bridge_node)

                        if bridge_new_nodes:
                            yield {
                                "event": "nodes_discovered",
                                "data": {"depth": 0, "nodes": [n.model_dump() for n in bridge_new_nodes]},
                            }
                    ...
                    if bridge_links:
                        logger.info(f"expand_all bridge: 发现 {len(bridge_links)} 条跨子图桥梁边")
                        yield {
                            "event": "links_discovered",
                            "data": {
                                "depth": 0,
                        ...
                        logger.warning("expand_all bridge: LLM 未返回任何桥梁边")
                except Exception as e:
                    logger.warning(f"expand_all bridge 失败: {e}")
            else:
                logger.info("expand_all: 展开后图已连通，无需桥梁发现")
