...
{{"links": [
  {{
    ...
    "relation": "upstream|downstream|substitute|cost_input|byproduct|logistics|competes",
    "impact": "neutral",
    "impact_reason": "具体描述两节点的产业关系（包含运输方式、成本占比、供需依赖度等）",
    "confidence": 0.85,
    "transmission_speed": "即时|1-3个月|3-6个月|半年以上",
    "transmission_strength": "强刚性|中等|弱弹性",
    "transmission_mechanism": "成本推动|供给收缩|需求替代|情绪传导|政策驱动",
...
class ChainAgent:
                    ...
                    for item_type, obj in items:
                        if item_type == "node":
                            ...
                            link = ChainLink(
                                source=source,
                                target=target,
                                relation=obj.get("relation", "upstream"),
                                impact=obj.get("impact", "neutral"),
    ...
    async def build(self, req: ChainBuildRequest):
        """构建产业链中性网络 — 真正流式推送
        ...
        for depth in range(1, req.max_depth + 1):
            ...
            prompt = CHAIN_BUILD_PROMPT.format(
                subject=root_name,
                focus_nodes="、".join(focus_list),
                existing_nodes="、".join(explored) if explored else "（无）",
                focus_area_note=focus_area_note,
            )
    ...
    def _parse_link_obj(
        ...
        """解析一个 link JSON 对象，名称归一化 + 自动补建缺失节点"""
    ...
    async def expand_all(self, leaf_nodes: list[str], existing_nodes: list[str]):
        ...
        async def _expand_one(node_name: str) -> list[dict]:
            ...
            async for evt in self.build(req):
                results.append(evt)
            return results
