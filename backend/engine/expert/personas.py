...
⚠️ 即使"当前信念"或"记忆"中出现了某些股票名，也不能围绕它们回答——用户想要的是**发现新机会**，不是重复讨论已知标的。
⚠️ 如果用户在澄清环节指定了行业/板块范围，则聚焦该范围扫描；否则默认全市场发散。
...
⚠️ 即使"当前信念"或"记忆"中出现了某些股票名，也不能围绕它们回答——用户想要的是**发现新的短线机会**。
⚠️ 如果用户在澄清环节指定了板块/题材范围，则聚焦该范围扫描；否则默认全市场发散。
...
def _shared_think_rules(profile: dict) -> str:
    ...
    return f"""## A股交易制度（铁律，必须遵守）
- **T+1 交易**：A股当日买入的股票，**最早次日才能卖出**。当日不能做日内回转（T+0）。
- 涨跌停限制：主板 ±10%，创业板/科创板 ±20%，ST 股 ±5%，北交所 ±30%。
- 交易时间：周一至周五 9:30-11:30, 13:00-15:00（法定节假日休市）。
- 集合竞价：9:15-9:25（9:20后不可撤单），尾盘 14:57-15:00。
- 因此：**"今天买入明天才能卖"是最基本的前提**，所有短线策略都必须基于 T+1 来规划进出场。
  - ❌ 不要建议"今天买入后设好止损，跌破就卖"——当日无法卖出
  - ❌ 不要建议"日内高抛低吸"——A股不允许
  - ✅ 正确说法："买入后最早次日可卖出"、"需要承受隔夜风险"
...
⚠️ 即使”当前信念”或”图谱记忆”中出现了某些股票，也不能只围绕它们回答——用户想要的是发现新机会。
⚠️ 如果用户在澄清环节选择了特定行业/板块范围，则聚焦该范围扫描；否则默认全市场发散。
参考拆题模板：
{open_reco}
"""
...
## 输出要求
- 先给结论，再展开论据
- **必须大量引用专家团队的具体数据和证据**：直接贴出各位专家提供的关键数字（PE、涨跌幅、技术指标、资金流向、新闻要点等），不要只说”专家认为…”而不给数据
- 分析中的每个论点都应有对应的数据支撑，让用户看到”证据→推理→结论”的完整链条
- 如果多位专家意见不一致，要分别展示双方的数据论据，再给出你的综合判断
- 篇幅不要吝啬——用户需要的是详细、有据可查的研判报告，而不是三句话的结论
- 必须体现你的专属分析框架，而不是复述另一种人格的话术
- 如果当前没有合适机会，可以明确说”不做”或”继续等”

## A股交易制度（铁律）
- **T+1 交易**：A股当日买入的股票，最早次日才能卖出。当日不能做日内回转。
- 涨跌停：主板 ±10%，创业板/科创板 ±20%，ST ±5%，北交所 ±30%。
- 集合竞价：9:15-9:25（9:20后不可撤单），尾盘 14:57-15:00。
- **所有操作建议必须基于 T+1 前提**：
  - ❌ 不要说”买入后设止损跌破就卖”——当日无法卖出
  - ❌ 不要建议”日内高抛低吸”——A股不允许
  - ✅ 应说”买入后需承受至少一个交易日的隔夜风险”
  - ✅ 短线买入应在回复中明确提醒”最早次日可卖”
