...
from engine.arena.schemas import Blackboard, DebateEntry, DataRequest, JudgeVerdict, RoundEval, RoundEvalSide, IndustryCognition
from engine.arena.personas import (
...
async def fetch_initial_data(
    ...
    if blackboard.target_type == "sector":
        INITIAL_ACTIONS: list[tuple[str, str, str, dict]] = [
...
ACTION_TITLE_MAP = {
    ...
    "get_capital_structure": "资金构成", "get_industry_cognition": "行业认知",
}
...
async def request_data_for_round(
    ...
    from engine.arena.personas import build_data_request_prompt
    context = _build_context_for_role(blackboard)
...
async def run_debate(
    ...
    if not blackboard.as_of_date:
        if blackboard.code:
            ...
            blackboard.as_of_date = _dt2.date.today().strftime("%Y-%m-%d")
    ...
    async for event in verdict_stream:
        ...
        if event["event"] == "judge_verdict":
            from engine.arena.schemas import JudgeVerdict as _JV
            judge_verdict = _JV(**{
